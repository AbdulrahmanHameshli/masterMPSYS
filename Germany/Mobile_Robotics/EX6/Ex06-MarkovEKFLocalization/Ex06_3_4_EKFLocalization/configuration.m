...
config.ENCERR = 0.01;       % ground truth angular displacement error, std in [rad]
% Laser scanner parameters and error modeling
config.ANGRESO = pi/180;      % range finder angular ANGRESOlution in [rad]
config.MAXR = 2.0;            % range finder perception radius in [m]
config.RSTD = 0.001;           % range standard deviation of range sensor. In [m]
config.CBEAC = 0.001*eye(2);   % beacon error model
config.PSENSORFAIL = 0;     % probability of a sensor failure
config.ALPHA = 0.95;
% Position controller frequency
config.DT = 0.3;
config.MANUALCONTROL = 1;