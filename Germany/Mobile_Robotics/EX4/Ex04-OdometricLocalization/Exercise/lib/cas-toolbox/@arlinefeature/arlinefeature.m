...
% v.1.0.1, Aug 2023, Kai Arras, Uni Stuttgart, see %%%
...
switch nargin
  ...
  case 3
    ...
    x = varargin{2};
    C = varargin{3};
    if isnumeric(x) && (numel(x) == 2) && isnumeric(C) && ...
       all(size(C) == [2,2]) && isnumeric(varargin{1})
      if size(x,1) == 1, x = x'; end   %%% make vertical 
      l.x = x;
      l.C = C;
