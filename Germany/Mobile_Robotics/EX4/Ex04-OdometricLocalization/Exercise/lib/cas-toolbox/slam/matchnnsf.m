...
% v.1.1, Kai Arras, Dec. 2014: add identical class test in line 42
...
if (nG > 0) && (nL > 0),
  ...
  for i = 1:nL,
    Cli = Cl(i,i).C;
    for j = 1:nG,
      if strcmp(class(Xl{i}),class(Xg{j})),
        Cgj = get(Xg{j},'c');
  ...
  for i = 1:nassoc,
    if associations(i).ig > 0,
      ...
      if (nl == 1) && (ng == 1),
        ...
        % alternative strategy: find strongest or nearest neighbor here
